...
class Admin_UnitsController extends Zend_Controller_Action
{
	...
	{            
		...
		$search = new Zend_Session_Namespace('units_search');
                $mask = $status = "";
        ...
        public function addAction()
	{
		...
		if( $this->getRequest()->isPost() ){
			...
			$this->_db->insert('units', $clean);
			$id = $this->_db->lastInsertId();
			$this->_redirect("/admin/units/");
		}
	}        
	...
	public function editAction()
	{
		...
		if(!empty($id) && is_numeric($id)){
			$sql = 'SELECT * FROM units WHERE id='.(int)$id.' LIMIT 1';
			...
			if( $this->getRequest()->isPost() ){
				...
				$this->_db->update('units', $clean, 'id='.$id );
				$this->_redirect("/admin/units");
			}
		}else{
			$this->_redirect('/admin/units');
	...
	public function deleteAction()
	{
		$id = $this->getRequest()->getParam('id');
		if(!empty($id) && is_numeric($id)){
			$sql = 'DELETE FROM units WHERE id='.(int)$id.' LIMIT 1';
			$this->_db->query($sql);
		}
		$this->_redirect('/admin/units');
	}
	...
	function init()
	{
		$this->view->placeholder('section')->set("units");
		...
		$this->view->formats = $this->_format;
