<?php
class Admin_KitchenController extends Zend_Controller_Action
{
	...
	{            
		...
		$select->from(array("kc"=>"kitchen_categories"), '*');
		//$select->where("status='enabled'");
		$select ->order(array("$sort $dir"));                
		if ($mask) {
			$search->mask = $mask;                        
			$select->where("kc.name LIKE '%$mask%'");
		}
		if ($status) {
			$search->status = $status;
			$select->where("kc.status = '$status'");
		}
        ...
        public function addAction()
	{
		...
		if( $this->getRequest()->isPost() ){
			...
			$this->_db->insert('kitchen_categories', $clean);
			$id = $this->_db->lastInsertId();
			$this->_redirect("/admin/kitchen");
		}
	}        
	...
	public function editAction()
	{
		...
		if(!empty($id) && is_numeric($id)){
			$sql = 'SELECT * FROM kitchen_categories WHERE id='.(int)$id.' LIMIT 1';
			...
			if( $this->getRequest()->isPost() ){
				...
				$this->_db->update( 'kitchen_categories', $clean, 'id='.$id );
				$this->_redirect("/admin/kitchen");
			}
		}else{
			$this->_redirect('/admin/kitchen');
	...
	public function deleteAction()
	{
		$id = $this->getRequest()->getParam('id');
		if(!empty($id) && is_numeric($id)){
			$sql = 'DELETE FROM kitchen_categories WHERE id='.(int)$id.' LIMIT 1';
			$this->_db->query($sql);
		}
		$this->_redirect('/admin/kitchen');
	}
	...
	function init()
	{
		$this->view->placeholder('section')->set("kitchen");
