<?php
class Admin_AppliancesController extends Zend_Controller_Action
{
	...
	{            
		...
		$select->from(array("ac"=>"appliance_categories"), '*');
        ...
        public function addAction()
	{
		...
		if( $this->getRequest()->isPost() ){
			...
			$this->_db->insert('appliance_categories', $clean);
			$id = $this->_db->lastInsertId();
			$this->_redirect("/admin/appliances/");
		}
	}        
	...
	public function editAction()
	{
		...
		if(!empty($id) && is_numeric($id)){
			$sql = 'SELECT * FROM appliance_categories WHERE id='.(int)$id.' LIMIT 1';
			...
			if( $this->getRequest()->isPost() ){
				...
				$this->_db->update('appliance_categories', $clean, 'id='.$id );
				$this->_redirect("/admin/appliances");
			}
		}else{
			$this->_redirect('/admin/appliances');
	...
	public function deleteAction()
	{
		$id = $this->getRequest()->getParam('id');
		if(!empty($id) && is_numeric($id)){
			$sql = 'DELETE FROM appliance_categories WHERE id='.(int)$id.' LIMIT 1';
			$this->_db->query($sql);
		}
		$this->_redirect('/admin/appliances');
	}
	...
	function init()
	{
		$this->view->placeholder('section')->set("appliances");
