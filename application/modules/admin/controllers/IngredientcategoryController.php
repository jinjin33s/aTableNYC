<?php
class Admin_IngredientcategoryController extends Zend_Controller_Action
{
	...
	{           
		...
		$select->from(array("fc"=>"ingredient_categories"), '*');
        ...
        public function addAction()
	{
		...
		$chef = $this->getRequest()->getParam('data', $this->_ingredientCategory);

		if( $this->getRequest()->isPost() ){
			$clean = $chef;

			$this->_db->insert('ingredient_categories', $clean);
			$id = $this->_db->lastInsertId();
			$this->_redirect("/admin/ingredientcategory/");
		}
	}        
	...
	public function editAction()
	{
		...
		if(!empty($id) && is_numeric($id)){
			$sql = 'SELECT * FROM ingredient_categories WHERE id='.(int)$id.' LIMIT 1';
			...
			if( $this->getRequest()->isPost() ){
				...
				$this->_db->update( 'ingredient_categories', $clean, 'id='.$id );
				$this->_redirect("/admin/ingredientcategory");
			}
		}else{
			$this->_redirect('/admin/ingredientcategory');
	...
	public function deleteAction()
	{
		$id = $this->getRequest()->getParam('id');
		if(!empty($id) && is_numeric($id)){
			$sql = 'DELETE FROM ingredient_categories WHERE id='.(int)$id.' LIMIT 1';
			$this->_db->query($sql);
		}
		$this->_redirect('/admin/ingredientcategory');
	}
	...
	function init()
	{
		...
		$this->_ingredientCategory = array(
                    'name'      => '',
                    'status'    => ''
		);                
